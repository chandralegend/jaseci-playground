...
const FileItem = ({ index, selected, fileName, onRename, onDelete }) => {
	...
	return !deleteMode ? (
		<div className={`file-entry ${selected && "active"}`}>
			<a
				href={`#file=${fileName}`}
				ref={contentRef}
		...
		<div className={`file-entry delete-mode`}>
			...
			},
			{
				...
				fn: (editor, { args = [] }) => {
					...
					if (fileName) {
						...
						editor.openNotification("Please specify a valid file name to write", {
							bottom: true,
							duration: 5000,
						});
					}
							...
							<div id='stdout'>
								{waiting ? (
									<div className='status-section loading'>
										<span className='icon'>i</span> running...
									</div>
								) : (
									<>
										{executionOutput.stdout && (
											<div className='output-box out'>
												...
												<pre>
													<code>{executionOutput.stdout}</code>
												</pre>
											</div>
										)}
										{executionOutput.stderr && (
											<div className='output-box err'>
												...
												<pre>
													<code>{executionOutput.stderr}</code>
												</pre>
											</div>
										)}
									</>
